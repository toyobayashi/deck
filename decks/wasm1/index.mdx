...
import perf22 from './assets/perf22.png'
import perf24 from './assets/perf24.png'
...
WASI 主要是为了能让 wasm 脱离浏览器运行在其他 wasm 运行时中

现阶段仍不支持 C++ 异常处理，pthread 多线程等常规功能

如果想在浏览器复用现有的 C 代码，推荐使用更加成熟的 Emscripten
...
```c subtitle="使用 Emscripten 编译时，WASM_EXPORT 应展开为 __attribute__((used))，编译命令如下"
// src/fib.c
...
// emcc -O3 --no-entry -o ./out/fibemcc.wasm ./src/fib.c
```
...
} = await fetchAndCompile('./out/fibemcc.wasm', {})
...
Emscripten 不但支持生成单独的 wasm，而且支持生成 JS 胶水代码

胶水代码中做的事情是初始化 wasm 内存，请求编译 wasm

往 importObject 中注入 C/C++ 系统库需要用到的 JS 实现等等

---

<CodeSurfer>

```js subtitle="编译命令使用 -sMODULARIZE 启用模块化，生成的文件名后缀改为 mjs"
...
```js subtitle="不需要手动 fetch 了，初始化返回 Emscripten 的 Module，C 中导出的符号带前缀下划线"
// emcc -O3 -sMODULARIZE -o ./out/fibemccesm.mjs ./src/fib.c

import init from './out/fibemccesm.mjs'

const Module = await init({ /* 初始化选项 */ })
const _fibemccesm = Module._fib
...
```js 8:10 subtitle="再用 JS 写一个动态规划优化过的斐波那契函数"
// fib.js
...
```js subtitle="动态规划，只用两个变量存储每一步的计算结果"
// fib.js
...
粗略对比一下刚才 WAT / 不同编译工具链编译的 c / JS 版本的斐波那契函数性能

<div style={{ display: 'flex' }}>
  <img src={perf22} />
  <img src={perf24} style={{ marginLeft: '64px' }} />
</div>

当输入 22 时 JS 慢 wasm 八倍，当输入 24 时 JS 慢 wasm 五六倍（JS 引擎本身会优化）

而 JS 动态规划优化后的算法仍比 wasm 快，可以看出好的算法才是性能的关键

---

# 感谢观看

源码：[https://github.com/toyobayashi/wasm-fib-demo](https://github.com/toyobayashi/wasm-fib-demo)
