...
fvi_result fvi_is_debug(fvi_t info, bool* result) {
  FVI_FFI_RETURN(
    ((info->ffi->dwFileFlags & info->ffi->dwFileFlagsMask) &
      VS_FF_DEBUG) != 0, false);
}
...
fvi_result fvi_string_file_info(fvi_t info,
                                ...
                                uint32_t* len) {
  if (id > 11 || result == NULL || len == NULL) {
    return fvi_invalid_result;
  }
  if (info == NULL) {
    *result = empty_string;
    *len = 0;
  ...
  HRESULT hr = StringCchPrintfW(sub_block, 64,
    string_file_info_keys[id], (*(info->trans)).wLanguage,
    (*(info->trans)).wCodePage);
  if (FAILED(hr)) return fvi_system_error;
  if (!VerQueryValueW(info->block, sub_block,
      (LPVOID*) result, len)) {
    *result = empty_string;
    *len = 0;
  } else {
